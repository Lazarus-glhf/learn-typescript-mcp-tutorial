...
Agent 评分时应优先检查 `works/chapter01/typed-toolbox-lab`。如果学习者已经在该目录中发起请求，也可以直接检查当前目录。
...
- 2 分：完成制造参数类型错误并修复的练习，且能说明 `pnpm typecheck` 报错含义
...
Agent 还要确认学习者完成过制造参数类型错误并修复的步骤。这个过程通常无法从最终文件直接证明；如果缺少证据，应追问 1-3 个短问题。重点看学习者是否能说明：
