...
- 10 分：项目目录名和基础结构正确，至少包含 `package.json`、`tsconfig.json`、`src/index.ts`
- 10 分：`src/index.ts` 能正确输出项目名和学习者信息
- 10 分：完成学习目标输出练习
- 10 分：完成“制造并修复类型错误”练习，并提交了真实错误信息
...
- 5 分：`package.json` scripts 清晰，包含 `dev`、`typecheck`、`build`、`start`
...
1. 当前终端是否在项目目录中
2. `package.json` scripts 是否写错
3. `tsconfig.json` 的 `module` / `moduleResolution` 是否匹配
4. `src/index.ts` 是否存在
5. 类型错误到底在说什么
