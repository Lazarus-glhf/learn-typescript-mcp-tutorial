...
Agent 评分时应优先检查 `works/chapter00/typed-toolbox-lab`。如果学习者已经在该目录中发起请求，也可以直接检查当前目录。
...
- 8 分：作业目录位于 `works/chapter00/typed-toolbox-lab` 或等价章节目录，基础结构正确，至少包含 `package.json`、`tsconfig.json`、`src/index.ts`
...
- 5 分：`tsconfig.json` 包含严格模式、合理的 `rootDir` / `outDir`，并在 `compilerOptions` 中启用 `"types": ["node"]`
...
Agent 还要检查 `tsconfig.json` 的 `compilerOptions` 中是否包含 `"types": ["node"]`。第 8 章开始会使用 `node:fs/promises`，第 12 章还会使用 `process.argv`；如果第 0 章缺少这一项，后续章节可能在 `pnpm typecheck` 时找不到 Node.js 内置 API 类型。
...
6. 当前终端是否在 `works/chapter00/typed-toolbox-lab` 或等价本章作业目录中
7. `package.json` scripts 是否写错
8. `tsconfig.json` 的 `module` / `moduleResolution` 是否匹配，且是否启用 `"types": ["node"]`
9. `src/index.ts` 是否存在
10. 类型错误到底在说什么
